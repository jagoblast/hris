...
authApi.post('/register', async (c) => {
  try {
    ...
    // Mendukung request dari Form HTML SSR
    else {
    ...
    // Generate ID unik, NIP otomatis, dan waktu
    const id = 'usr_' + Date.now().toString(36);
    const nip = 'ADM-' + new Date().getFullYear() + '-' + Math.floor(1000 + Math.random() * 9000);
    const today = new Date().toISOString().split('T')[0];
    const now = new Date().toISOString(); // Waktu saat ini untuk kolom created_at
    
    // Eksekusi insert ke database Cloudflare D1 (Menambahkan created_at)
    await db
      .prepare(`
        INSERT INTO users 
        (id, nip, name, email, password_hash, role, position, department, phone, base_salary, allowance_transport, allowance_meal, join_date, leave_quota, avatar, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .bind(
