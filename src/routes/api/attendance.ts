...
import { db, getSettingValue } from '../../db/d1';
...
attendanceApi.get('/today', async (c) => {
  ...
  const workStartTime = await getSettingValue('WORK_START_TIME', '08:30');
  const workEndTime = await getSettingValue('WORK_END_TIME', '17:30');
  const companyName = await getSettingValue('COMPANY_NAME', 'PT Nusantara Digital Pratama');
...
attendanceApi.post('/check-in', async (c) => {
  try {
    ...
    // Load work schedule from database settings
    const workStartTime = await getSettingValue('WORK_START_TIME', '08:30');
    const [cfgH, cfgM] = workStartTime.split(':').map(Number);
    const officeStartMinutes = (cfgH || 8) * 60 + (cfgM || 30);
    ...
    if (currentTotalMinutes > officeStartMinutes) {
      isLate = 1;
    ...
    const companyName = await getSettingValue('COMPANY_NAME', 'PT Nusantara Digital Pratama');
    const location = body.location ?? `Kantor Pusat ${companyName}`;
    ...
    return c.json({
      success: true,
      message: isLate
        ? `Check-in berhasil. Perhatian: Anda terdeteksi terlambat ${lateMinutes} menit (Batas Masuk: ${workStartTime.substring(0, 5)} WIB).`
