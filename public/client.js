...
// 12. Save Company & Work Hours Settings to D1
window.saveCompanySettings = async function(event) {
  ...
  try {
    window.showToast('Menyimpan ke tabel settings Cloudflare D1...', 'info');
    ...
    const json = await res.json();
    if (json.success) {
      window.showToast(json.message || 'Pengaturan berhasil diperbarui di database D1!', 'success');
      setTimeout(() => window.location.reload(), 1000);
    } else {
      window.showToast(json.error || 'Gagal menyimpan pengaturan', 'error');
      if (btn) btn.disabled = false;
    }
  } catch (err) {
    window.showToast('Gagal menghubungi server', 'error');
