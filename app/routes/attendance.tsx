...
export async function AttendancePage(user: JWTPayload) {
  ...
  // Fetch settings from D1 database
  const settings = await getAllSettingsMap();
  const companyName = settings['COMPANY_NAME'] || 'PT Nusantara Digital Pratama';
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-zinc-400">Pencatatan jam kerja GPS, deteksi keterlambatan setelah ${workStartTime} WIB, dan jam pulang ${workEndTime} WIB.</p>
        </div>
        ...
        <div class="p-6 rounded-3xl bg-gradient-to-br from-zinc-800 to-zinc-900 border border-zinc-700/80 flex flex-col justify-between shadow-sm space-y-4">
          <div>
            ...
            <div class="text-center p-4 rounded-2xl bg-black/40 border border-zinc-700/60 my-2">
              ...
              <div class="text-[10px] text-orange-400 font-semibold mt-1">Batas Tepat Waktu: ${workStartTime}:00 WIB</div>
            </div>

            <div class="space-y-2 mb-3 text-xs">
              <div class="flex items-center justify-between p-2.5 rounded-xl bg-zinc-900 border border-zinc-800">
                <span class="text-zinc-500">Perusahaan:</span>
                <span class="font-semibold text-zinc-200 truncate max-w-[180px]">${companyName}</span>
              </div>
              <div class="flex items-center justify-between p-2.5 rounded-xl bg-zinc-900 border border-zinc-800">
